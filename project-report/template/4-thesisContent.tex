...
The ElGamal KEM is defined by the following algorithms and explained in section\ref{sec:elgamal}:

\paragraph{Key Generation.}
\begin{algorithmic}[1]
\STATE $\mathsf{KeyGen}(\mathcal{G}) \rightarrow (pk, sk)$:
...
\begin{algorithmic}[1]
\STATE $\mathsf{Enc}(pk) \rightarrow (K, C)$:
...
\begin{algorithmic}[1]
\STATE $\mathsf{Dec}(C, sk) \rightarrow K$:
...
\begin{lstlisting}[
  style=easycrypt,
  caption={ElGamal KEM Implementation in EasyCrypt \protect\footnotemark},
  breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true
]
...
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L223C1-L243}}
...
For a KEM scheme $\mathsf{KEM} = (\mathsf{KeyGen}, \mathsf{Enc}, \mathsf{Dec})$ and adversary $\mathcal{A}$, the IND-CCA1 security game proceeds as follows(detailed explained in section\ref{subsec:indcca}:
...
The key component in our formalization is the algebraic oracle that enforces AGM constraints through a two-layer design followed by the insights from paper ~\cite{fuchsbauer2018}: 

\paragraph{Interface Layer: Decryption Oracle Abstraction under AGM.}

We encode the phase-based constraints of the IND-CCA1 game directly. The decryption oracle interface \texttt{Oracles\_CCA1i} provides initialization \texttt{init} and representation based decryption \texttt{dec(c, z)}, where vector $z$ represents the algebraic representation of ciphertext group element $c$ relative to the visible base list $l$. The adversary interface \texttt{Adv\_INDCCA1} is decomposed into scout phase \texttt{scout(pk)} (allowing calls to \texttt{O.dec}) and a distinguishing phase \texttt{distinguish(k, c)} (prohibiting calls to \texttt{O.dec}).
...
\begin{lstlisting}[style=easycrypt, caption=Algebraic Oracle Interface \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
...
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L75-L79}}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L170-L173}}
...
The module \texttt{O\_CCA1\_Limited} enforces query quota and representation consistency validation at the implementation level. In \texttt{dec(c, z)} calls, a query is considered valid only when the quota is not exceeded and the condition $c = \texttt{prodEx}(l, z)$ is satisfied. For valid queries, the oracle calls the underlying scheme's decryption algorithm \texttt{S.dec} to obtain the session key and incorporates it into the base list $l$ and query log $qs$. For invalid queries, a placeholder value \texttt{witness} is returned to avoid additional leakage.
...
\begin{lstlisting}[style=easycrypt, caption=Limited Oracle Implementation with Validation \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
...
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L120-L154}}


\section{q-DDH Assumption}
\label{sec:q-ddh-assumption}

As we explained the background of q-DDH in the section\ref{sec:part2qddh}, it's main property can be conclude as following sentence: In particular, even with access to $g^x, g^{x^2}, \ldots, g^{x^q}$, it should be infeasible for any efficient adversary to distinguish a true Diffie-Hellman pair from a randomly generate one~\cite{ddh2015}. 
...
\begin{lstlisting}[style=easycrypt, caption=q-DDH Game Implementation \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
...
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L250-L287}}
...
Then, the key part of the game module \texttt{QDDH(A)} is presented. It first declares exponent field elements $x$, $r$, $z$, $b\_int$, a group element list $gtuple$, a single group element $challenge$, and boolean variables $b$, $b'$. The program then independently and uniformly samples from distribution $dt$ to obtain the secret exponent $x$, the random exponent $r$ used for generating the challenge, and the additional random quantity $z$, and flips a coin to get $b$.

The program converts the boolean value $b$ to an element $b\_int$ in the exponent field. Subsequently, it generates the list $gtuple$ using \texttt{map (fun i => g\^(exp x i)) (range 1 (q+1))}, which computes term by term according to the exponential powers $x^1, x^2, \ldots, x^q$ to obtain $[g^x, g^{x^2}, \ldots, g^{x^q}]$.

The challenge element $challenge$ is constructed through: $g^{(x \cdot r) + (z \cdot b\_int)}$. Therefore, when $b$ is true ($b\_int = 0$), the challenge is $g^{xr}$, and when $b$ is false ($b\_int = 1$), the challenge is $g^{xr+z}$.
...
Our formalization builds upon a linear algebra framework that enables manipulation of algebraic representations in our entire work while preserve accurate correspondence with group operations.
These mathematical structures are fully implemented and verified within the \textsc{EasyCrypt} proof assistant, giving a machine-checked foundation for the algebraic reasoning used in subsequent proof.
\subsection{Core Linear Algebra Components}
...
\subsubsection{Basic Group Operations.}

\begin{lstlisting}[style=easycrypt, caption=Basic Group Operations \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
...
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L83-L91}}
...
\subsubsection{Vector Operations in the Exponent Field.}

\paragraph{Zero Vector (\texttt{zerov})}
...
\begin{lstlisting}[style=easycrypt, caption=Zero Vector Definition \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
(* Zero vector of length q+1 *)
op zerov = nseq (q+1) zero.
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L316}}


This serves as the additive identity for vector operations and the initial value for vector accumulation.

\paragraph{Vector Addition (\texttt{addv})}

Vector addition enables pointwise addition of corresponding elements from two vectors:
...
\begin{lstlisting}[style=easycrypt, caption=Vector Addition \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
(* Vector addition: pointwise addition *)
op addv (a b : exp list) = 
  map (fun (x : exp * exp) => x.`1 + x.`2) (zip a b).
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L319}}


This operation is commutative and associative, forming the basis for linear combinations.

\paragraph{Vector Multiplication (\texttt{mulv})}

Vector multiplication serve as pointwise ultiplication of corresponding elements:

\begin{equation}
\texttt{mulv}(a, b) = [a_1 \cdot b_1, a_2 \cdot b_2, \ldots, a_n \cdot b_n]
\end{equation}

\begin{lstlisting}[style=easycrypt, caption=Vector Multiplication \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
(* Vector multiplication: pointwise multiplication *)
op mulv (a b : exp list) = 
  map (fun (x : exp * exp) => x.`1 * x.`2) (zip a b).
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L322}}

This operation is used for combining exponent vectors when computing products of group elements.

\paragraph{Scalar Multiplication (\texttt{scalev})}
...
\begin{lstlisting}[style=easycrypt, caption=Scalar Multiplication \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
(* Scalar multiplication: multiply vector by scalar *)
op scalev (a : exp list)(b : exp) = map (fun x => x*b) a.
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L325}}

This operation enables scaling of algebraic representations, crucial for constructing linear combinations in this work.

\paragraph{Vector Sum (\texttt{sumv})}
...
\begin{lstlisting}[style=easycrypt, caption=Vector Sum \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
(* Sum of vectors: fold addition over list of vectors *)
op sumv (a : exp list list) = foldr addv zerov a.
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L328}}
...
\paragraph{Shift and Truncate (\texttt{shift\_trunc})}
...
\begin{lstlisting}[style=easycrypt, caption=Shift and Truncate Operation \protect\footnotemark, breaklines=true, breakatwhitespace=true, frame=single, keepspaces=true]
(* Shift and truncate operation for oracle simulation *)
op shift_trunc (v : exp list) = take (q+1) (zero :: v).
\end{lstlisting}
\footnotetext{code link :\url{https://github.com/GuSheldom/INDCCA-QDDH/blob/main/INDCCA.ec\#L331}}
...
\subsection{Key Algebraic Properties and Lemmas}

The correctness of our reduction relies on several fundamental algebraic properties and lemmas shows below, which are essential for our work.
...
\begin{lemma}[Prodex size less or equal]
\label{lem:prodex-sizele}
When the exponent vector is longer than the base list, the product only depends on the prefix matching the base list length:
\begin{align}
\mathsf{prodEx}(bases, exps) &= \mathsf{prodEx}(bases, \mathsf{take}(|bases|, exps)) 
\quad \text{when } |exps| \geq |bases|
\end{align}

Equivalently, oversized exponents beyond the base list size are ignored:
\begin{align}
\prod_{i=0}^{n-1} g_i^{e_i} &= \prod_{i=0}^{n-1} g_i^{e_i} 
\quad \text{when } |[e_0, e_1, \ldots, e_m]| \geq n \text{ and } m \geq n
\end{align}
\end{lemma}
\noindent \textit{Proof:}  Section~\ref{sec:prodex-sizele-proofs}.
...
or group element $x$, group list $xs$, exponent $e$, and exponent list $es$:
\[
\mathsf{ex}(x :: xs, e :: es) = (x^e) :: \mathsf{ex}(xs, es)
\]

\end{lemma}
\noindent \textit{Proof:} See Section~\ref{sec:sumv-cons-proofs}.


\begin{lemma}[Sumv Cons Distribution]
\label{lem:sumv-cons-general}
F
to adding the first element to the sum of the remaining elements, we can get:
\[
